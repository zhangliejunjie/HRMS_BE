...
const handleGetJobsByCampaignId = async (req, res) => {
  if (!req.body.campaignId) {
    return res.status(404).json({ errMsg: "campaignId not found" });
  }
  let message = await jobService.getJobsByCampaignId(req.body.campaignId);
  return res.status(200).json(message);
};
